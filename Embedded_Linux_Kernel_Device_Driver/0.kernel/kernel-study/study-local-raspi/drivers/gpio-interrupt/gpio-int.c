/**
*       gpio-int.c
*       Simple GPIO driver example for interrupt
...
#define GPIO_21_OUT (533)

//LED is connected to this GPIO
//#define GPIO_25_IN  (25)
#define GPIO_25_IN  (537)
...
static int __init gpio_driver_init(void)
{
        ...
        if((alloc_chrdev_region(&dev, 0, 1, "gpio_int_example")) < 0) {
        ...
        /*Creating device*/
        if(IS_ERR(device_create(dev_class, NULL, dev, NULL, "gpio_int"))) {
        ...
        if (request_irq(gpio_irq,             //IRQ number
        ...
        pr_info("Device Driver (gpio-int.ko) Insert...Done!!!\n");
        return 0;
...
static void __exit gpio_driver_exit(void)
{
        ...
        cdev_del(&gpio_cdev);
        unregister_chrdev_region(dev, 1);
        pr_info("Device Driver (gpio-int.ko) Remove...Done!!\n");
}
